  ...
  async fetch(request, env) {
    ...
    if (url.pathname === '/api/campaigns/create' && request.method === 'POST') {
      try {
        ...
        return new Response(JSON.stringify({ success: true, campaign_id: row.id, cpu, budget, message: 'Campaign live' }), { headers: cors });
      } catch (e) { return new Response(JSON.stringify({ error: e.message }), { status: 500, headers: cors }); }
    }
    ...
    if (url.pathname === '/api/trials/start' && request.method === 'POST') {
      try {
        ...
        if (campaign.budget_spent >= campaign.budget) return new Response(JSON.stringify({ error: 'Budget exhausted' }), { status: 400, headers: cors });
        if (campaign.owner_github === ghUser.login) return new Response(JSON.stringify({ error: 'Cannot trial your own product' }), { status: 400, headers: cors });
        await env.DB.prepare(
        ...
        return new Response(JSON.stringify({ success: true, product_url: campaign.product_url, milestone: campaign.milestone, reward_cents: campaign.cpu }), { headers: cors });
      } catch (e) { return new Response(JSON.stringify({ error: e.message }), { status: 500, headers: cors }); }
    }
    ...
    if (url.pathname === '/api/trials/complete' && request.method === 'POST') {
      try {
        ...
        if (now - trial.started_at < 60) return new Response(JSON.stringify({ error: 'Too fast — spend 60 seconds trying the product' }), { status: 400, headers: cors });
        ...
        return new Response(JSON.stringify({ success: true, earned_cents: trial.payout_cents, earned_dollars: (trial.payout_cents / 100).toFixed(2) }), { headers: cors });
      } catch (e) { return new Response(JSON.stringify({ error: e.message }), { status: 500, headers: cors }); }
    }
    ...
    if (url.pathname === '/api/earnings' && request.method === 'POST') {
      try {
        ...
        const { results: trials } = await env.DB.prepare(
          `SELECT t.*, c.title as campaign_title, c.cpu FROM user_trials t
